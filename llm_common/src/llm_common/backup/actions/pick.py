...
class PickObjectAction(object):
    ...
    def pick(
        ...
        force: float = 1.0,
        pre_grasp: float = 0.1,
        post_grasp: float = 0.2,
        open_distance: float = 1.2,
        upper_offset: float = 0.00,
    ):
        """
        ...
        self.whole_body.move_end_effector_pose(
            hsrb_interface.geometry.Pose(
                pos=np.array(target.worldpos()), ori=matrix2quaternion(target.rotation)
            ),
...
if __name__ == "__main__":
    ...
    dummy_target = Coordinates(
        ...
        rot=[
            -1.1161014323551877e-05,
            0.9979444148772441,
            0.06408528958065665,
            -0.00014265381319705783,
        ],
    )
