...
class PlacementFinder(object):
    ...
    def subscribe(self):
        ...
        sub_polygon = message_filters.Subscriber(
            self.polygon_topic_name, jsk_recognition_msgs.msg.PolygonArray, queue_size=1
        )
        ...
        if self.approximate_sync:
            ...
            sync = message_filters.TimeSynchronizer(fs=self.subs, queue_size=queue_size)
    ...
    def get_polygons(self, with_message=False):
        ...
        for index, (coeff, polygon) in enumerate(
            zip(coeffs_msg.coefficients, polygons_msg.polygons)
        ):
    ...
    def get_rects(
        self, polygons_msg=None, coeffs_msg=None, boxes_msg=None, indices=None
    ):
        ...
        for index, (coeff, polygon) in enumerate(
            ...
            points = np.array(
                [[point.x, point.y, point.z] for point in polygon.polygon.points],
                dtype=np.float32,
            )
            normal = [a, b, c]
            projected_points = rotate_points(points, normal, [0, 0, 1])
            shapely_polygon = shapely.geometry.Polygon(projected_points)
            ...
            rect_polygon = rotate_points(rect_polygon_2d, [0, 0, 1], normal)
    ...
    def get_poses(
        ...
        for index, (coeff, polygon) in enumerate(
            ...
            for box in boxes:
                ...
                projected_vertices = rotate_points(box.vertices, normal, [0, 0, 1])
            ...
            points = points_within(
                shapely_polygon.buffer(-margin), box_polygons=shapely_box_polygons
            )

            orientation_coords = skrobot.coordinates.Coordinates()
            skrobot.coordinates.geo.orient_coords_to_axis(orientation_coords, -normal)
            q_wxyz = orientation_coords.quaternion
            output_poses = []
            for p in points:
                x, y, z = rotate_points(np.array([p.x, p.y, -d]), [0, 0, 1], normal)[0]
                output_poses.append(Coordinates(pos=(x, y, z), rot=q_wxyz))
