...
def main():
    ...
    cmd = "{}/run_docker -host pr1040 -launch vlpart_segment.launch vocabulary:=custom \"classes:='bottle cap; cup handle;'\"".format(pkg_path)
    ...
    subprocess.run("docker kill $(docker ps -aqf \"ancestor=tracking_ros:latest\")", shell=True)
