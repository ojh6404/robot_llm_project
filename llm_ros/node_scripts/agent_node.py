...
class AgentNode(object):
    def __init__(self):
        ...
        self.sub_task_desc = rospy.Subscriber(
            "/llm_agent/task/description", Message, self.task_callback, queue_size=1
        )

    def task_callback(self, task_msg):
        print(task_msg.message)
        response_text = self.agent.query(
            role="user",
            new_prompt=task_msg.message,
            messages=[{"role": "system", "content": task_msg.message}],
        )
